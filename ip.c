...
ip_iface_alloc(const char *unicast, const char *netmask)
{
    ...
    ip_addr_t uaddr;
    if (ip_addr_pton(unicast, &uaddr) == -1) {
        errorf("unable to convert unicast");
        memory_free(iface);
        return NULL;
    }
    ip_addr_t naddr;
    if (ip_addr_pton(netmask, &naddr) == -1) {
        errorf("unable to convert netmask");
        memory_free(iface);
        return NULL;
    }
    iface->unicast = uaddr;
    iface->netmask = naddr;
    iface->broadcast = uaddr | ~naddr;
    return iface;
}
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if (0 != sum) {
        errorf("checksum does not match");
        return;
    }
    offset = ntoh16(hdr->offset);
